...
// Meerdere aanvragers van hetzelfde product staan samen in het `requester`-veld,
// gescheiden door " & " (bijv. "Erik & Steve"). Geen aparte tabel/kolom nodig
// voor iets wat verder nergens los van elkaar gebruikt wordt.
export function splitRequesters(requester) {
  return (requester || '')
    .split('&')
    .map((name) => name.trim())
    .filter(Boolean);
...
export function includesRequester(requester, name) {
  if (!name) return false;
  return splitRequesters(requester).some(
    (n) => n.toLowerCase() === name.trim().toLowerCase(),
  );
}

function mergeRequesters(requester, name) {
  const names = splitRequesters(requester);
 ...
 *
 * Vraagt iemand hetzelfde product nog eens aan - ongeacht wie het eerder al
 * aanvroeg - dan gaat het aantal omhoog en komt de naam erbij te staan
 * ("Erik & Steve"), in plaats van een aparte regel. Zo ziet de besteller in
 * één oogopslag het totaal, en wie er allemaal interesse hebben.
 */
export async function addItem(db, data) {
  ...
  const existing = await db
    ...
    .bind(data.productId, data.productId, data.productId, data.title)
    .first();

  if (existing) {
    const merged = Math.min(existing.quantity + quantity, MAX_QUANTITY);
    const requester = mergeRequesters(existing.requester, data.requester);
    await db
      .prepare(
        'UPDATE items SET quantity = ?, requester = ?, note = COALESCE(NULLIF(?, \'\'), note) WHERE id = ?'
      )
      .bind(merged, requester, data.note || '', existing.id)
      .run();
    return {
      item: { ...rowToItem(existing), quantity: merged, requester, note: data.note || existing.note },
      merged: true,
    };
  }
...
// Volledige reset: elke rij weg, ongeacht status - het schema en de indexen
// blijven staan, dus geen nieuwe D1-setup nodig. Onomkeerbaar.
export async function wipeAll(db) {
  const result = await db.prepare('DELETE FROM items').run();
...
// Top van wat er organisatiebreed het vaakst besteld is (dus status 'ordered'),
// duplicaten samengevoegd op product - voor het globale statistiekenoverzicht.
export async function topProductsOverall(db, limit = 10) {
  const { results } = await db
    .prepare(
      `SELECT product_id AS productId, title, brand, unit_size AS unitSize,
              image_url AS imageUrl, product_url AS productUrl, price,
...
// Het product dat het vaakst is aangevraagd, over alle statussen heen (dus ook
// nog open, ingetrokken of afgewezen verzoeken tellen mee) - puur een leuk
// weetje, geen bestelstatistiek.
export async function mostRequestedProduct(db) {
  const row = await db
    .prepare(
      `SELECT product_id AS productId, title, brand, unit_size AS unitSize,
              image_url AS imageUrl, product_url AS productUrl,
              SUM(quantity) AS totalQuantity, COUNT(*) AS times
       FROM items
       GROUP BY COALESCE(product_id, lower(title))
       ORDER BY totalQuantity DESC, times DESC
