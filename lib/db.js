...
const SELECT_FIELDS = `
  ...
  unit_size, image_url, product_url, price, quantity, note, status
`;
...
export async function listItems(db, status) {
  ...
  const order = status === 'open' ? 'created_at ASC' : 'ordered_at DESC, created_at DESC';
  const { results } = await db
    .prepare(`SELECT ${SELECT_FIELDS} FROM items WHERE status = ? ORDER BY ${order}`)
    .bind(status)
    .all();
...
export async function setStatus(db, id, status) {
  ...
  const result = await db
    .prepare('UPDATE items SET status = ?, ordered_at = ?, deleted_at = ? WHERE id = ?')
    .bind(status, orderedAt, deletedAt, id)
    .run();
